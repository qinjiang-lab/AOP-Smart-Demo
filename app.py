...
import markdown  # 新增：用于将Markdown转为HTML
...
if st.session_state.last_result:
    ...
    with col1:
        if st.button("🗑️ Clear Result"):
            ...
            st.rerun()
    with col2:
        # 使用 markdown 库将 Markdown 内容转换为 HTML（保留 <a> 标签）
        md_content = st.session_state.last_result
        # 注意：last_result 中已经包含 <a> 标签，这些是 HTML，markdown 库会原样保留
        # 同时 Markdown 表格会被解析为 HTML 表格
        ...
        <head>
            <meta charset="UTF-8">
            <title>AOP Analysis Report</title>
            <style>
                body {{
                ...
                h2 {{ color: #2c3e50; margin-top: 28px; border-bottom: 1px solid #ddd; padding-bottom: 6px; }}
                h3 {{ color: #34495e; margin-top: 20px; }}
                a {{ color: #1f77b4; text-decoration: none; font-weight: 500; }}
        ...
        <body>
            {html_body}
            <div class="footer">
                Generated by <strong>AOP-Smart</strong> — {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
            </div>
        </body>
        </html>
        """
        ...
        st.download_button(
            ...
            data=html_content,
            file_name=f"AOP_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html",
            mime="text/html"
        )
